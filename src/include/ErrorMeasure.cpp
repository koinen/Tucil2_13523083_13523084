...
double ErrorMeasure::SSIMThreshold(const Image& image, int x1, int y1, int x2, int y2) {
    Image original;
    original.loadImage("test/example.png");
    // C1 from 8-bit, normalized (0-1)
    double C1 = pow(0.01 * 1, 2);

    // C2 from 8-bit, normalized (0-1)
    double C2 = pow(0.03 * 1, 2);
    
    // FOR CURRENT IMAGE
    double redMean = image.Red().block(y1, x1, y2 - y1 + 1, x2 - x1 + 1).mean();
    double greenMean = image.Green().block(y1, x1, y2 - y1 + 1, x2 - x1 + 1).mean();
    double blueMean = image.Blue().block(y1, x1, y2 - y1 + 1, x2 - x1 + 1).mean();
    ...
    // FOR ORIGINAL IMAGE
    double originalRedMean = original.Red().block(y1, x1, y2 - y1 + 1, x2 - x1 + 1).mean();
    double originalGreenMean = original.Green().block(y1, x1, y2 - y1 + 1, x2 - x1 + 1).mean();
    double originalBlueMean = original.Blue().block(y1, x1, y2 - y1 + 1, x2 - x1 + 1).mean();

    double originalRedVar = Variance(original.Red(), x1, y1, x2, y2);
    double originalGreenVar = Variance(original.Green(), x1, y1, x2, y2);
    double originalBlueVar = Variance(original.Blue(), x1, y1, x2, y2);

    // SSIM
    double redCov = Covariance(image.Red(), original.Red(), x1, y1, x2, y2);
    double greenCov = Covariance(image.Green(), original.Green(), x1, y1, x2, y2);
    double blueCov = Covariance(image.Blue(), original.Blue(), x1, y1, x2, y2);
